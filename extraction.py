# # During the internship period implemented code with the paid version
...
def get_gemini_response(input_text: str, image=None, max_retries=2):
    """Get response from Gemini with automatic retry on rate limits using multiple free models"""
    
    models_to_try = [
        # Primary models (when quota available)
        'gemini-2.5-flash-lite',     # 10 RPM, 20 RPD
        'gemini-3-flash',             # 5 RPM, 20 RPD (might be unavailable)
        'gemini-2.5-flash-tts',       # 3 RPM, 10 RPD (might be unavailable)
        
        # Gemma models - YOUR WORKHORSES! 14.4K RPD each
        'gemma-3-27b-it',             # 30 RPM, 14.4K RPD ✓ WORKING ✓ WORKING
        'gemma-3-12b-it',             # 30 RPM, 14.4K RPD
        'gemma-3-4b-it',              # 30 RPM, 14.4K RPD
        'gemma-3-1b-it',              # 30 RPM, 14.4K RPD
        'gemma-3-2b-it',              # 30 RPM, 14.4K RPD
        
        # Other options
        'gemini-robotics-er-1.5-preview',  # 10 RPM, 20 RPD
        'gemini-1.5-flash',            # Original fallback
    ]
    
    for model_name in models_to_try:
        # Skip embedding model for text generation (it's for embeddings only)
        ...
        for attempt in range(max_retries):
            try:
                ...
                if image is not None:
                    # Check if model supports images (Gemma models might not)
                    if 'gemma' in model_name.lower():
                        # Gemma models are text-only
                        response = model.generate_content(input_text)
                        logger.info(f"Using {model_name} for text-only (image ignored)")
                    else:
                ...
                if "429" in error_str or "quota" in error_str.lower():
                    # Rate limit hit - try next model immediately
    ...
    # If all models fail, try one more time with gemini-1.5-flash as ultimate fallback
    try:
        logger.info("🔄 Ultimate fallback: trying gemini-1.5-flash")
...
def get_gemini_model_response(input_text):
    """
    Standalone function to get response from Gemini API and return the used model name as a string.
    Displays the model name (gemini-2.5-flash-lite) via Streamlit toast and print.
    Returns a string in the format 'Model used: {model_name}' or 'Model used: None' if the model fails.
    """
    def get_today_date():
        return datetime.utcnow().strftime("%Y-%m-%d")
...
# Initialize jd from text input
jd = jd_text.strip()  # ← Keep this

# Process JD images (don't reset jd)
# Process JD images (don't reset jd)
image_texts = []
if jd_images:
    extracted_texts = []
    ...
    if extracted_texts:
        image_text = "\n---\n".join([text.strip() for text in extracted_texts if text.strip()])
        # Combine with text input if it exists
        if jd:
            jd = jd + "\n---\n" + image_text
        else:
            jd = image_text
...
# NEW: Detect JD language and translate to English if not English
if jd:
    detected_language = detect_language(jd)
    st.info(f"Detected JD language: {detected_language}")
            ...
            with st.spinner("Analyzing resume..."):
                ...
                try:
                    if exp_response:
                        ...
                        logger.warning("No response from experience analysis")
                except Exception as e:
                    logger.error(f"Error parsing experience analysis: {str(e)}")
                    st.error(f"Error parsing experience analysis: {str(e)}")
                    # Keep default values
                ...
                # Initialize education variables with better defaults
                edu_score = 1.0  # Default to perfect score if no requirements
                edu_explanation = "No specific education requirements found in JD"
                matched_education = extracted_resume.get("education", [])
                missing_education = []

                try:
                    if edu_response and edu_response.strip():
                        # Try to parse the response
                        edu_result = json.loads(edu_response)
                        edu_score = edu_result.get("score", 1.0)
                        edu_explanation = edu_result.get("explanation", "Education analysis completed")
                        matched_education = edu_result.get("matched_education", extracted_resume.get("education", []))
                        missing_education = edu_result.get("missing_education", [])
                    else:
                        logger.info("No education response, using defaults")
                        
                        # Check if JD has education requirements
                        jd_education = extracted_jd.get("education", [])
                        if jd_education:
                            # If JD has requirements but no response, assume partial match
                            edu_score = 0.5
                            edu_explanation = "Could not analyze education requirements in detail"
                        else:
                            # No JD requirements = perfect score
                            edu_score = 1.0
                            edu_explanation = "No education requirements specified in JD"
                            
                except json.JSONDecodeError as e:
                    logger.error(f"Error parsing education analysis: {e}")
                    # Check if JD has education requirements
                    jd_education = extracted_jd.get("education", [])
                    if jd_education:
                        edu_score = 0.5
                        edu_explanation = "Education requirements exist but could not be analyzed"
                    else:
                        edu_score = 1.0
                        edu_explanation = "No education requirements in JD"
                except Exception as e:
                    logger.error(f"Unexpected error in education analysis: {e}")
                    # Keep defaults

                # Initialize variables for Step 8
                skills_explanation = "Skills analysis completed"
                effective_weights = section_weights.copy()

                # Step 8: Adjust weights for skills (ONLY ONCE!)
                jd_skills = extracted_jd.get("required_technical", []) + extracted_jd.get("required_soft", [])
                if not jd_skills:
                    skills_score = 1.0
                    matched_skills = []
                    ...
                    # Skills score should already be calculated from Step 4
                    # Make sure these variables exist
                    if 'skills_score' not in locals():
                        skills_score, matched_skills, missing_skills = get_skills_match_via_gemini(
                            extracted_resume.get("technical_skills", []),
                            extracted_resume.get("soft_skills", []),
                            extracted_jd.get("required_technical", []),
